	...
	void parseSettings(
		...
		auto readConfigLine = [&]() {
			...
			if(skipWhitespaces(ln)) {
				...
				else if(key == u8"cel-exponent")                    UL_ parseRealNumber(&dst->shadeStepExponent, lnv);
				...
				else if(key == u8"point-light-intensity-threshold") UL_ parseRealNumber(&dst->pointLightIntensityThreshold, lnv);
