...
namespace vkutil {
	...
	CommandPool::CommandPool(VkDevice dev, uint32_t fam_index):
			CommandPoolBase(dev)
	{
		...
		vk_check("vkCreateCommandPool", vkCreateCommandPool, mVkDevice, &cp_info, nullptr, &mPool);
	}
	...
	void CommandPool::runAsync(const CommandInfo& cmd_info, const RunFn& fn) {
