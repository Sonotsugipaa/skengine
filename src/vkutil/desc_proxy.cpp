...
namespace vkutil {
	...
	uint_fast64_t desc_gen     = uint_fast64_t(DsetToken::eInvalid);
	auto          desc_gen_sem = std::binary_semaphore(1);
	...
	DsetToken DescriptorProxy::makeUniqueToken() noexcept {
		desc_gen_sem.acquire();
		auto r = DsetToken(++ desc_gen);
		if(r == DsetToken::eInvalid) [[unlikely]] {
			return DsetToken(++ desc_gen);
		}
		desc_gen_sem.release();
		return r;
	}
	...
	DsetToken DescriptorProxy::createToken(VkDescriptorSetLayout layout) {
		...
		if(mInvalidated) {
			mTokens.insert({ r, { layout, VK_NULL_HANDLE } });
		} else {
	...
	VkDescriptorSet DescriptorProxy::resolveToken(
			DsetToken token,
			const PoolResetSyncCallback& reset_sync_fn
	) {
	...
	void DescriptorProxy::writeDescriptorSets(uint32_t desc_count, const VkWriteDescriptorSet* writes) {
		(void) desc_count;
		(void) writes;
		const char* error_msg = "Requested to write a descriptor set, which is not implemented yet";
